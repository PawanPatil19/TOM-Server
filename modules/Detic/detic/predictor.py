...
class VisualizationDemo(object):
    ...
    def run_on_video(self, video):
        ...
        def process_predictions(frame, predictions):
            ...
            pred_classes = frame_predictions['instances'].pred_classes.cpu().numpy()
            confidence = frame_predictions['instances'].scores.cpu().numpy()
            bounding_boxes = frame_predictions['instances'].pred_boxes.tensor.cpu().numpy()
            for i in pred_classes:
                prediction_class_names.append(self.metadata.thing_classes[i])

            # Converts Matplotlib RGB format to OpenCV BGR format
            vis_frame = cv2.cvtColor(vis_frame.get_image(), cv2.COLOR_RGB2BGR)
            return vis_frame, prediction_class_names, confidence, bounding_boxes
